...
function setup() {
 ...
 angleMode(DEGREES);
  ...
  subBut.position(1*width/10,2*height/20);
...
function draw() {
  ...
  drawArrow(xStart+(LineX-xStart)/2,yStart+10,xStart,yStart+10);

  drawArrow(xStart+(LineX-xStart)/2,yStart+10,LineX,yStart+10);
...
function heightValue(){
  ...
  if(LineX>xStart+Hypo*cos(xAngle)*cos(yAngle)-2 && LineX<xStart+Hypo*cos(xAngle)*cos(yAngle)+2)
    text('cos(x) X cos(y)',xStart+(LineX-xStart)/2,yStart+25);
  else if(LineX<xStart+Hypo*cos(xAngle)*cos(yAngle)-2 || LineX>xStart+Hypo*cos(xAngle)*cos(yAngle)+2)
    text('cos(x) X '+ m,xStart+(LineX-xStart)/2,yStart+25);
